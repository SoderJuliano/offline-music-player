...
export class MySubClassedDexie extends Dexie {
  ...
  constructor() {
    ...
    this.version(4).stores({
      ...
      return tx.table('songs').toCollection().modify(async (song) => {
...
class DbService {
  ...
  private async _ensureOpen() {
    try {
      ...
      throw new Error('O acesso ao banco de dados falhou. Verifique se o navegador não está no modo privado ou com configurações de privacidade muito restritas.');
    }
  }
